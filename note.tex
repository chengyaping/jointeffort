...
derived using scintillation light.\todo {appropriate?}
...
scintillator needs to be $\ge$ \hl{20}m at 430 nm.\todo {more specific}
%\begin{center}
...
water Cerenkov detectors, such as Super-K.\todo{add reference here}
...
from  the vertex to the hits on the detector Eq.~(\eqref{recmethod:1}). \todo{
    add notation defination here}
%here,$\vec{d_0}$is the particle direction,$\vec{O_0}$is the vertex position
%derived from vertex reconstruction algorithm. $\vec{P_i}$ is the positon and 
%$q_i$ is the charge of the PMT. 
...
\todo {need add the performance of the other two methods on this plot}
...
simulation was done under the sniper framework~\cite{sniper}.\todo{???}
...
detector response correction.\todo{this part should be more clear }
\begin{figure}[htbp]
...
%position and PMT  facing  direction. 
...
\begin{figure}[htbp]
\centering % \begin{center}/\end{center} takes some additional vertical space
\includegraphics[width=.75\textwidth]{plots/centerresult}
% "\includegraphics" is very powerful; the graphicx package is already loaded
...
%\end{tabular*}%
%\end{center}
\todo{need update should be consistent with the plot}
...
%Positron direction information can be reconstructed from above 
%procedures , here 1ns PMT tts is supposed.
...
to neutron vertex. %SN direction uncertainty is $11.21^{\circ}$.
...
\clearpage
\appendix
\section{key materials necessary to reproduce the simulation}
